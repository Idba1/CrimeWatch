            ...
            <div className="overflow-hidden relative">
                <div className="flex" ref={containerRef} style={{ minHeight: '100%', whiteSpace: 'nowrap', overflowX: 'hidden' }}>
                    {stories.map((story) => (
                        ...
                        >
                            <motion.div
                                className="w-full h-64 bg-orange-300 bg-center bg-cover rounded-lg shadow-md"
                                style={{ backgroundImage: `url(${story.imageUrl})` }}
                            ></motion.div>

                            <div className="w-56 -mt-10 overflow-hidden bg-white  shadow-lg md:w-64 dark:bg-orange-500">
                                <h3 className="py-2 font-bold tracking-wide text-center text-orange-500 uppercase dark:text-white">{story.name}</h3>
                                {/* <p className="px-3 py-2 text-center text-orange-700 dark:text-orange-300">{` ${story.companyName}`}</p> */}
                            </div>
                        </motion.div>
                    ))}
                </div>
